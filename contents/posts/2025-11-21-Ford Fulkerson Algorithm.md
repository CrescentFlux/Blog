...
- **定义**
```
Ford-Fulkerson方法是一种用于在容量网络中计算从源点到汇点的最大流的算法框架,只要不断找增广路径并推流，直到找不到为止;不指定如何找路径（DFS、BFS、随机都可以）
```
- **定义**
```
Edmonds-Karp是这个框架的具体实现:明确要求必须用BFS找增广路径,获得了多项式时间保证
标准Edmonds-Karp特征:必须用BFS找最短路径边数最少;每次找一条增广路径;立即推流并更新剩余图;重复直到找不到路径
```
...
- **最大流最小割定理**
  ...
  - 割:
    - 一个割是二元组 (S', T')，其中：S' 包含源点S,T' 包含汇点T,S' ∪ T' = 所有节点,S' ∩ T' = ∅割的容量 = 所有从S'指向T'的边的容量和
    - 割是把所有节点分成两个集合：包含S的集合 和 包含T的集合，然后计算从S集合到T集合的所有边的容量和
    - 选择一组边，使得删除这些边后，从S到T没有任何路径
    - 割是任意的节点划分，只要S和T在不同侧;每个割都代表一种完全断流的方案;最小割就是代价最小的断流方案;
...
class FlowNetwork {
    ...
    addEdge(u, v, capacity) {
        ...
        if (this.graph[v][u] === undefined) {//只在反向边不存在时初始化为0，避免覆盖
        this.graph[v][u] = 0; //确保反向边初始化为0⚠️这会覆盖已存在的边！这里总是把反向边设为0，但应该检查是否已存在
    }
    }
    deepCopyGraph() {
        const copy = {};
...
👉反向边不是物理通道，而是重新分配权限的数学记录：反向边就是"记忆+减法"机制
反向边实际上维护了节点流量平衡：算法通过反向边 = 在保持总量不变的前提下重新分配,最终效果 = 不浪费任何容量，找到全局最优
虚拟反向边就是一个会计技巧，不是物理连接它让算法能够在不违反物理容量限制的情况下，重新分配各条路径的责任，从而找到更好的整体方案。
👉没有反向边 = 贪心算法，容易陷入局部最优;有反向边 = 系统性的全局最优搜索
反向边容量 = 实际已使用的流量,反向边表示可以重新分配的量，你不能重新分配你根本没有使用过的容量
反向边容量 = 在这条路径上实际用掉的流量,不能超过边的原始容量,不能超过这条路径实际运送的流量,必须是真实使用过的量
在标准最大流中，反向边只对已有边创建,反向边只针对原始图中实际存在的边创建,如果原始图没有A→B边，就不会有B→A反向边
👉Ford-Fulkerson 的核心思想:Ford-Fulkerson 在1956年提出
反复寻找增广路径（Augmenting Path），并沿其推流，直到找不到为止;能增则增，增无可增即为最大; 
Ford-Fulkerson = 在剩余图中不断找路送水 + 用反向边留后悔余地 + 无路可走时停止//自动找到最大流量，不管网络多复杂
增广路径就是：在剩余图中，从源点S到汇点T的一条路径，并且这条路径上的每条边都有剩余容量 > 0从起点到终点，能找到一条所有路段都还有空车位的完整路线
在网络流中，capacity 表示一条边能够承载的最大流量,网络流模型里我们考虑的是任意时刻的瞬时流量不能超过容量,网络流这个数学模型里，我们要求同一时刻不能有
*/
...
/*
👉BFS的访问标记是基于节点，不是基于边
BFS的工作方式：BFS追求最短路径：不需要走回头路
const visited = new Set([source]); // 开始时就标记S已访问
// 处理队列：
1. 处理S → 发现S→A, S→B → 标记A,B已访问
2. 处理A → 检查A→S: S已访问 → 跳过
          → 检查A→T: T未访问 → 继续
检查边 A→S, 容量: 0, 已访问: true
👉每次BFS都是全新的搜索,每次BFS创建新的visited集合所有节点都重新标记为"未访问"//每次BFS都基于最新的剩余图重新搜索
剩余图改变了：边的容量更新了;路径可能不同：上次不能走的路径现在可能能走;需要重新探索：算法要找的是当前剩余图中的新路径
*/
}
...
//网络太简单，没有需要"绕路"的场景,反向边在更复杂的网络中才会发挥作用,在S→A→T网络中，一旦A→T满了，就没有其他路径了
// 测试2：并行路径
...
console.log('👉关键测试结果:', networkCritical.fordFulkerson('S', 'T'));

//👉经典的反向边演示网络
const networkClassic = new FlowNetwork();
networkClassic.addEdge('S', 'A', 10);
networkClassic.addEdge('S', 'B', 10);
networkClassic.addEdge('A', 'B', 5);   // 关键边！
networkClassic.addEdge('A', 'T', 5);
networkClassic.addEdge('B', 'T', 15);
console.log('👉经典测试结果:', networkClassic.fordFulkerson('S', 'T'));
/* 智能重分配
决策1：S→A→T (流量5)
决策2：S→A→B→T (流量5) ← 利用反向边调整
结果：S→A的10容量全部利用！
出口总容量 = 20,关键在于 A→B 这条边允许流量重分配
路径1: S → A → T (流量5)     // A→T用满
路径2: S → B → T (流量10)    // B→T用10
路径3: S → A → B → T (流量5) // A把多余的5转给B
验证：
A节点：流入10(S→A)，流出5(A→T) + 5(A→B) = 10 ✓
B节点：流入10(S→B) + 5(A→B)，流出15(B→T) = 15 ✓  
T节点：流入5(A→T) + 15(B→T) = 20 ✓
最大流回答能做到多好;最小割回答为什么不能更好;定理保证这两个答案一致
最大流20：系统的性能上限;最小割20：系统的瓶颈所在;系统的最大能力 = 系统的最弱环节
*/
