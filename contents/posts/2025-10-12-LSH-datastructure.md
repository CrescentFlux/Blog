...
|方面	|向量相似性 (Similarity)|	向量距离 (Distance)|
---|---|---
|数学关系	|相似性 = 1 - 距离|	距离 = 1 - 相似性|
|取值范围|	[0, 1] 或 [-1, 1]	|[0, +∞)|
|直观理解|	值越大越相似|	值越小越相似|
|LSH中使用|	余弦相似性 (主流)|	欧氏距离、曼哈顿距离|
|计算方式	|`cos(θ) = A·B / (‖A‖‖B‖)`|`√Σ(Aᵢ - Bᵢ)²`|
|对长度敏感度|	不敏感（方向更重要）|	敏感（绝对位置重要）|
|LSH超平面原理|	基于向量方向相似|	基于向量位置接近|
|业务意义	|方向是否一致|	实际位置是否靠近|
